...
class Mailjet
{
    ...
    var $apiKey = '228eef14c7866d827038bc8e43ebd493';
    var $secretKey = 'c089b13f307368b7a4dd1fc9b88a023c';
...
class NewsletterController extends Controller{
    ...
    public function accueilAction()
    {
      ...
      //  $userInfo = $client->get(RequestApi::USER_INFOS);
       ...
       /* $response=$client->post(RequestApi::LISTS_ADD_CONTACT,
            array(
       ...
       return $this->render('ChaireAccueilBundle:Newsletter:newsletter.html.twig',array('page_active'=>0));

    }

    public function addtomailjetAction()
    {

        if ($this->get('request')->request->get('email') != null && $this->get('request')->request->get('who') != null) {

            if ($this->get('request')->request->get('who') > 0 && $this->get('request')->request->get('who') < 6) {
                $email = $this->get('request')->request->get('email');


                switch ($this->get('request')->request->get('who')) {
                    case 1:
                        $idlist = 1388281;
                        break;
                    case 2:
               ...
               if ($response!=null)
               {
                   $this->get('session')->getFlashBag()->add(
                       'notice',
                       'Votre e-mail à bien été ajouté, Merci de votre confiance !'
                   );
                    return $this->redirect($this->generateUrl('chaire_accueil_homepage'));
               }
        ...
        $this->get('session')->getFlashBag()->add(
            'error',
            'Erreur lors de votre inscription à la newsletter, avez vous rentré votre e-mail correctement ?'
        );
            return $this->redirect($this->generateUrl('chaire_accueil_homepage'));
